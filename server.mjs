...
async function chat(req, res) {
  ...
  try {
    ...
    for await (const chunk of req) {
      raw += chunk;
      if (raw.length > 30000) return json(res, 413, {error:'Message is too long.'});
    }
    const data = JSON.parse(raw);
    if (!Array.isArray(data.messages) || !data.messages.length || data.messages.length > 11 || data.messages.some(m => !['user','assistant'].includes(m?.role) || typeof m.content !== 'string' || (m.role === 'user' && m.content.length > 500))) return json(res, 400, {error:'Invalid message.'});
    const input = data.messages.map(m => ({role:m.role, content:m.content.slice(0, m.role === 'assistant' ? 1500 : 500)}));
